...
struct Em10Work {
    ...
    Vec Goto_pos;             // 0x4EC (0x8CC)  destination of RouteCkToPos, L_go measured to it (PS2 Goto_pos)
    ...
    Vec x5E0;             // 0x5E0 (0x9C0)  action target pos / approach offset (scaled 0.2 per frame)  action target pos / approach offset (scaled 0.2 per frame)
    u32 Goto_mode;             // 0x5EC (0x9CC)  ckGoto  ckGoto (PS2 Goto_mode)
    Vec x5F0;             // 0x5F0 (0x9D0)  goto-mode destination (player / bell), copied to Goto_pos  goto-mode destination (player / bell), copied to Goto_pos
    ...
    u16 x648;             // 0x648 (0xA28)  em10ParasiteAtkCk wait (Rnd % 300 + 300)  em10ParasiteAtkCk wait (Rnd % 300 + 300)
    u8 pad_64A[2];
    ...
    u16 x654;             // 0x654 (0xA34)  claw attack retry wait (150 / 450 after Dm_Claw)  claw attack retry wait (150 / 450 after Dm_Claw)
    u16 x656;             // 0x656 (0xA36)  frames the player has been lost (flags 0x800000)  frames the player has been lost (flags 0x800000)
