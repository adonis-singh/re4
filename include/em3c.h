...
// cModel::type variants share the routines: types 1 / 3 use the second motion set (`female`), types
...
struct Em3cWork {
    ...
    u8 pad_8[4];
    f32 turnAng;          // 0x00C (0x3EC)  em3c_R1_Turn180: target yaw
    int actMode;          // 0x010 (0x3F0)  em3c_R1_AtkWait: action button variant (1 / 2, 0 = none)
    u8 pad_14[0xC];
    f32 routeAng;         // 0x020 (0x600)  Muku towards the route point
    f32 routeAngAbs;      // 0x024 (0x604)
    u8 pad_28[8];
    f32 targetAng;        // 0x030 (0x610)
    f32 targetAngAbs;     // 0x034 (0x614)
    f32 L_go;       // 0x038 (0x618)
    f32 L_pl_route;           // 0x03C (0x61C)  RouteCkPosToPosDis to the player
    Vec routePos;         // 0x040 (0x620)  RouteCkToPos result
    u8 pad_4C[0xC];
    Vec targetPos;        // 0x058 (0x638)
    cEm* pTarget;         // 0x064 (0x644)  the player
    u8 pad_68[0xC];
    ...
    u8 female;            // 0x3DC (0x7BC)  type 1 / 3: second motion set
...
#define EM3C_BOMB(p) ((Em3cPartsBomb*) &(p)->addRot)
