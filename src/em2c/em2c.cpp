...
static void em2c_R1_F_Wait(cEm2c* em)
{
    ...
    case 1:
        ...
        if (w->targetAngAbs > 2.09439516f) {
            em->setRno(1, 3, wait, wait);
        } else {
            em->setRno(1, 0x18, wait, wait);
        }
        break;
    }
    if (em2cFallCk(em)) {
...
static void em2c_R1_W_Walk(cEm2c* em)
{
    ...
    case 1:
        ...
        if (plPos.z < -8000.0f) {
            em->setRno(1, 0x1F, t, t);
        }
        r = em2cWallFallCk(em);
        if (r) {
            em->setRno(1, 0x20, t, t);
            break;
        }
    ...
    case 1:
        ...
        if (MotionMove(em, 0)) {
            if (Rnd() % 10 > 4) {
                ...
                em->setRno(1, 0x1D, w->atkHit, w->atkHit);
            }
        } else if (em->Motion.Seq_old.Free & 1) {
            if (em->r_no_3) {
                em2cAtkCk(em, 3, 0xB);
...
static void em2c_R1_Dm_Guard(cEm2c* em)
{
    ...
    case 1:
        if (MotionMove(em, 0)) {
            ...
            if (w->targetAngAbs > 2.09439516f) {
                em->setRno(1, 3, lock, lock);
            } else if ((w->flags & 1) && (Rnd() & 1) && w->targetAngAbs < 0.52359879f) {
                em->setRno(1, 2, lock, lock);
            } else {
                em->setRno(1, 1, 0, 0);
            }
            break;
